(() => {
	...
	// Conference-specific schema and rules
	const CONFERENCE_SCHEMA = `
	  SELECT
		-- Columns from tbl_staffdev_conference_info
		t1.conference_ID AS conference_ID_t1,
		t1.conference_Name, -- Name of conference
		t1.conference_Desc, -- Description
		t1.Institute_Agency,
		t1.conference_Venue,
		t1.Start_Date,
		t1.End_Date,
		...
		-- Columns from tbl_staffdev_conference
		t2.Emp_Code AS Emp_Code_t2,
		t2.conference_ID AS conference_ID_t2,
	  ...
	  FROM 
		tbl_staffdev_conference_info t1
	  LEFT JOIN 
		tbl_staffdev_conference t2 ON t1.conference_ID = t2.conference_ID
	  LEFT JOIN 
	...
	const SYSTEM_PROMPT = `You are a SQL expert specialized in conference data. 
